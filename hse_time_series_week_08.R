...
# 01. Заполнение пропусков
air = as_tsibblle(AirPassengers)
gg_tsdisplay(air)
...
lair_ar1 = na_kalman(lair_na,
                     model = mod)
ggplot_na_imputations(lair_na, lair_ar1, lair)

lair_ar2 = na_kalman(lair_na,
                     model = 'auto.arima')
ggplot_na_imputations(lair_na, lair_ar2, lair)

lair_seas = na_seadec(lair_na)
ggplot_na_imputations(lair_na, lair_seas, lair)

# 02. Обнаружение выбросов
library(tidyverse)
...
ggplot(data = air2, aes(x = date, y = pass)) +
  geom_line() 

air3 = mutate(air2, ln_pass = log(pass))
ggplot(data = air3, aes(x = date, y = ln_pass)) +
  geom_line() 
...
ggplot(data = air4, aes(x = date, y = ln_pass_an)) +
  geom_line() 
...
ggplot(data = decomp4, aes(x = date, y = observed_cleaned)) +
  geom_line() 

# 03. Обнаружение структурного сдвига в R
library(rio)
...
ggplot(data = rm3, aes(x = date, y = deaths)) +
  geom_line() 
